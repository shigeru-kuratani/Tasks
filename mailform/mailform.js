...
$(function() {
	...
	$(document).on("click", "#confirm", function() {
		...
		$("#mailaddress").removeClass("alert-danger");
		...
		if (content.replace(/^(\s)+/, "").replace(/^( )+/, "")
				　　.replace(/(\s)+$/, "").replace(/(　)+$/, "") == "") {
			errorElm.push($("#content"));
			errorMessage += "問合せ内容を入力してください<br>";
		}

		// エラー判定
		if (errorMessage !== "") {
			let formMessageElm = $("#formMessage");
			formMessageElm.removeClass("alert-warning");
			formMessageElm.addClass("alert-danger");
			formMessageElm.html(errorMessage);

			for (i in errorElm) {
				errorElm[i].addClass("alert-danger");
			}
	...
	$(document).on("click", "#send", function() {
		...
		// ajax通信
		let url = "https://sm.skuratani.net";
		$.ajax({
			type     : "POST",
			url      : url,
			data     : data,	
